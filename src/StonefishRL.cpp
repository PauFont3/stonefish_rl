...
void StonefishRL::MostrarValors() {
    ...
    for (const auto& name : relevant_obs_names_) {
        ...
        else if(actuators_.count(name) > 0) {
            std::cout << "[INFO] " << name << " is an actuator in the scenario." << std::endl;
        }
        else if(robots_.count(name) > 0) {
            std::cout << "[INFO] " << name << " is a robot in the scenario." << std::endl;
        }
        else {
            std::cout << "[WARN] " << name << " is not a sensor, actuator or robot in the scenario." << std::endl;
        }
    }
}
